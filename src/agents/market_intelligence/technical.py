...
from ...data.schemas import AgentRole, TechnicalReport, Sentiment
...
class TechnicalAnalyst(BaseAgent):
    ...
    async def analyze(self, context: Dict[str, Any]) -> TechnicalReport:
        ...
        try:
            ...
            current_macd = float(price_data['MACD'].iloc[-1]) if 'MACD' in price_data.columns else None
...
"""
            ...
            indicators_dict = {
                ...
                "rsi": current_rsi,
                "macd": current_macd,
                "sma_20": float(price_data['SMA_20'].iloc[-1]) if 'SMA_20' in price_data.columns else None,
                "sma_50": float(price_data['SMA_50'].iloc[-1]) if 'SMA_50' in price_data.columns else None,
                "sma_200": float(price_data['SMA_200'].iloc[-1]) if 'SMA_200' in price_data.columns else None,
